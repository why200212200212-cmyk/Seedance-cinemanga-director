...
- 只提交当前条USE镜头需要的素材；宫格/故事板按剧情选择PANEL-ID，只提交同格CLEAN单格，PAGE/PANEL-REVIEW路线层先转成自然语言后剔除；SKIP不提交，REFERENCE-ONLY不占独立视频时长；当前VID色卡如提交则标为REFERENCE-ONLY COLOR，并同时在提示词写关键P编号/HEX/对象；
- 参考图预算不足时优先角色身份、当前执行分镜和关键空间/首尾帧，色卡降级为文字PAL合同；
