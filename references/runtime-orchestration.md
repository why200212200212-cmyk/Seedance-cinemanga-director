# Agent 运行编排

本 Skill 的导演能力与执行端分离，因此可被 Codex、OpenClaw 或其他能够读取 AgentSkills、执行 Python 命令的 AI 使用。平台无需实现专有插件，只需把仓库作为 Skill 加载，并在实际生成时提供火山方舟凭证。

## 标准流水线

1. **剧情分析**：读取剧本、参考图与用户约束，建立事实表、人物清单、剧情节拍和时间预算。
2. **角色注册**：每个必要角色单独生成多视角角色板，建立永久ID、参考图编号和身份注册表。
3. **分镜编排**：选择标准故事板、九宫格、二十五宫格或智能宫格；建立USE、REFERENCE-ONLY、SKIP清单和AI可辨识路线。
4. **导演编译**：只加载当前 3D 或真人专项规则，把USE镜头编译为导演方案与可复制视频提示词；排除SKIP和制作标注。
5. **硬门槛质检**：检查剧情忠实度、角色绑定、镜头取舍、路线、时间预算、物理可信度、表演、声音与连续性。
6. **请求预检**：建立当前条执行素材清单，把可复制视频提示词写入临时 UTF-8 文件，按角色板、分镜页、场景板和尾帧的声明顺序构建 API 请求，并先执行 `--dry-run`。
7. **用户授权**：只有用户明确要求实际生成且凭证、模型权限、费用预期均已确认，才创建任务。
8. **异步执行**：创建任务，轮询至终态；成功时下载成片，失败时返回官方错误，不虚构结果。
9. **成片复核**：如果执行环境能读取视频或抽帧，按 `references/quality-checklist.md` 复核身份、动作、空间、路线结果、光影、对白与尾帧。需要重试时先指出具体失败项，再最小幅度修订提示词；不得无上限自动重试付费任务。

## 平台无关调用

若 Codex、OpenClaw 或其他宿主已经提供由用户配置的 Seedance/火山方舟工具，可将下述创建、查询、取消与下载动作映射到该工具，但仍必须遵守同一字段顺序、授权和费用边界。没有现成工具时，在仓库根目录统一调用：

```bash
python scripts/seedance_client.py create --prompt-file <临时提示词文件> --dry-run
python scripts/seedance_client.py create --prompt-file <临时提示词文件> --wait --output <目标视频路径>
```

多参考图按执行素材清单顺序重复传入 `--image-url`。每个角色必须使用独立角色板；当前条不需要的角色、SKIP镜头与文档装饰图不得提交。账户开放新的多模态格式时使用 `--content-json`，不得自行猜测字段。

## AI 行为边界

- 不在回答、日志或命令行参数中回显 API key；优先从 `.env` 或环境变量读取。
- 不把“导演方案（审阅用）”、状态账本或隐藏推理提交给生成模型。
- 不因用户说“做到极致”而堆叠矛盾镜头、不可拍运动或空泛画质词；极致来自可执行的摄影、表演、材质、光照、声音与连续性约束。
- 不把 API 成功等同于成片合格；生成结果仍需按当前模式专项门槛检查。
- 不在没有用户实际生成指令时调用可能计费的创建接口。
