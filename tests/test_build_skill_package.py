...
class BuildSkillPackageTests(unittest.TestCase):
    def test_package_is_minimal_installable_and_reproducible(self) -> None:
        with tempfile.TemporaryDirectory() as directory:
            ...
            self.assertIn(prefix + "templates/multi-fighter-action.md", names)
            self.assertIn(prefix + "templates/cinematic-department-review.md", names)
            self.assertIn(prefix + "templates/color-palette-board.md", names)
            self.assertIn(prefix + "templates/compiled-video-prompt.md", names)
            ...
            self.assertIn(prefix + "references/multi-fighter-action-design.md", names)
            self.assertIn(prefix + "references/martial-arts-action-sources.md", names)
            self.assertIn(prefix + "references/cinematic-department-review.md", names)
